...
#[test]
fn selection_moves_whole_columns_and_preserves_edge_order() -> Result<(), Error> {
    let matrix = Matrix::new(
        [10, 11, 12, 13, 14],
        [('a', vec![1, 2, 3, 4, 5]), ('b', vec![6, 2, 8, 9, 0])],
    )?;
    let selected = matrix.select([4, 1, 3])?;
...
#[test]
fn empty_dimensions_keep_their_distinct_meanings() -> Result<(), Error> {
    let matrix = Matrix::new([1, 2], [(0, vec![3, 4]), (1, vec![3, 5])])?;
    let empty = matrix.select([])?;
    assert_eq!(empty.edges(), [0, 1]);
    assert_eq!(empty.rows().collect::<Vec<_>>(), [(0, vec![]), (1, vec![])]);
    let no_predecessors = Matrix::<u8, _>::new([1], [])?;
    assert_eq!(no_predecessors.columns()[0].common_argument(), None);
    assert_eq!(no_predecessors.columns()[0].arguments(), []);
    assert!(Matrix::<u8, u8>::new([], [])?.rows().next().is_none());
    Ok(())
}

#[test]
fn malformed_inputs_and_selections_are_rejected() -> Result<(), Error> {
    ...
    let matrix = Matrix::<u8, _>::new([1, 2], [])?;
...
#[test]
fn chained_selection_is_equivalent_to_composed_selection() -> Result<(), Error> {
    let matrix = Matrix::new([10, 11, 12, 13], [(0, vec![0, 1, 2, 3]), (1, vec![4, 5, 6, 7])])?;
    assert_eq!(matrix.select([3, 0, 2])?.select([2, 0])?, matrix.select([2, 3])?);
    Ok(())
}
