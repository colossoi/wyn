...
"#;
    ...
    let defunctionalized = tlc_transformed.partial_eval().normalize_soacs().fuse_maps().defunctionalize();
    let monomorphized = defunctionalized.monomorphize();

    // Convert to SSA using the new direct path
    let ssa_converted = monomorphized.buffer_specialize().inline().to_ssa().expect("SSA conversion failed");
