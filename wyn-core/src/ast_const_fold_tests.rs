...
fn make_binop(op: BinaryOperator, lhs: Expression, rhs: Expression) -> Expression {
    Expression {
        h: test_header(),
        kind: ExprKind::BinaryOp(BinaryOp { op }, Box::new(lhs), Box::new(rhs)),
    }
}
...
fn test_fold_simple_addition() {
    ...
    let mut expr = make_binop(BinaryOperator::Add, make_int(2), make_int(7));
...
fn test_fold_nested_arithmetic() {
    let mut folder = AstConstFolder::new();
    // (2 + 3) * 4 = 20
    let inner = make_binop(BinaryOperator::Add, make_int(2), make_int(3));
    let mut expr = make_binop(BinaryOperator::Multiply, inner, make_int(4));
    folder.fold_expr(&mut expr);
...
fn test_fold_with_constant_reference() {
    ...
    // X + 3 = 8
    let mut expr = make_binop(BinaryOperator::Add, make_ident("X"), make_int(3));
    folder.fold_expr(&mut expr);
...
fn test_no_fold_non_constant() {
    ...
    let mut expr = make_binop(BinaryOperator::Add, make_ident("y"), make_int(3));
    folder.fold_expr(&mut expr);
...
fn test_division_by_zero_not_folded() {
    let mut folder = AstConstFolder::new();
    let mut expr = make_binop(BinaryOperator::Divide, make_int(10), make_int(0));
    folder.fold_expr(&mut expr);
...
fn test_fold_slice_start_constant() {
    ...
    let start = make_binop(BinaryOperator::Add, make_int(1), make_int(2));
...
fn test_fold_slice_end_constant() {
    ...
    let end = make_binop(BinaryOperator::Add, make_int(5), make_int(5));
...
fn test_fold_slice_both_components() {
    ...
    let start = make_binop(BinaryOperator::Add, make_int(1), make_int(1));
    let end = make_binop(BinaryOperator::Multiply, make_int(4), make_int(2));
    ...
    if let ExprKind::Slice(slice) = &expr.kind {
        ...
        assert_eq!(
            start.kind,
            ExprKind::IntLiteral("2".into()),
            "start should fold to 2"
        );
...
fn test_fold_slice_with_constant_inlining() {
    ...
    if let ExprKind::Slice(slice) = &expr.kind {
        ...
        assert_eq!(
            start.kind,
            ExprKind::IntLiteral("2".into()),
            "start should inline to 2"
        );
        assert_eq!(
            end.kind,
            ExprKind::IntLiteral("10".into()),
            "end should inline to 10"
        );
    } else {
        panic!("Expected Slice, got {:?}", expr.kind);
    }
...
fn test_zero_minus_float_becomes_negation() {
    ...
    let mut expr = make_binop(BinaryOperator::Subtract, make_float(0.0), make_ident("x"));
    folder.fold_expr(&mut expr);

    if let ExprKind::UnaryOp(op, operand) = &expr.kind {
        assert_eq!(op.op, UnaryOperator::Negate, "Should be negation operator");
        assert!(
...
fn test_zero_minus_int_becomes_negation() {
    let mut folder = AstConstFolder::new();
    // 0 - y should become -y
    let mut expr = make_binop(BinaryOperator::Subtract, make_int(0), make_ident("y"));
    folder.fold_expr(&mut expr);
...
fn test_nonzero_minus_does_not_become_negation() {
    ...
    let mut expr = make_binop(BinaryOperator::Subtract, make_float(1.0), make_ident("x"));
    folder.fold_expr(&mut expr);
