...
use super::ssa_soac_analysis::{analyze_program, ArrayProvenance, ParallelizableMap};
...
fn parallelize_entry(
    ...
    let params: Vec<(Type<TypeName>, String)> = entry
        .inputs
        .iter()
        .map(|i| (i.ty.clone(), i.name.clone()))
        .collect();
    ...
    let zero = builder
        .push_int("0", i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;

    // Get storage length via intrinsic
    let set_val = builder
        .push_int(&set.to_string(), i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let binding_val = builder
        .push_int(&binding.to_string(), i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    ...
    let output_binding_val = builder
        .push_int(&output_binding.to_string(), i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    ...
    let total_threads_val = builder
        .push_int(&total_threads.to_string(), i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let threads_minus_1 = builder
        .push_int(&(total_threads - 1).to_string(), i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let len_plus = builder
        .push_binop("+", storage_len, threads_minus_1, i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let chunk_size = builder
        .push_binop("/", len_plus, total_threads_val, i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;

    // 5. chunk_start = thread_id * chunk_size
    let chunk_start = builder
        .push_binop("*", thread_id, chunk_size, i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;

    // 6. chunk_end = min(chunk_start + chunk_size, len)
    let start_plus_size = builder
        .push_binop("+", chunk_start, chunk_size, i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    ...
    let unit_val = builder
        .push_inst(InstKind::Unit, unit_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    builder
    ...
    let cond = builder
        .push_binop("<", loop_blocks.index, chunk_end, bool_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    builder
    ...
    let one = builder
        .push_int("1", i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let next_i = builder
        .push_binop("+", loop_blocks.index, one, i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    builder
        .terminate(Terminator::Branch {
            target: loop_blocks.header,
...
fn create_single_thread_fallback(
    ...
    let zero = builder
        .push_int("0", i32_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
    let is_thread_zero = builder
        .push_binop("==", thread_id, zero, bool_ty.clone(), dummy_span, dummy_node_id)
        .ok()?;
