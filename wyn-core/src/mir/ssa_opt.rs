            ...
            match term {
                ...
                Terminator::CondBranch {
                    ...
                    pred_map
                        .entry(*then_target)
                        .or_default()
                        .push((src, then_args.clone()));
                    pred_map
                        .entry(*else_target)
                        .or_default()
                        .push((src, else_args.clone()));
                }
                _ => {}
            }
        }
...
fn apply_substitutions(body: &mut FuncBody, subs: &HashMap<ValueId, ValueId>) {
    ...
    for inst in &mut body.insts {
        match &mut inst.kind {
            ...
            | InstKind::ArrayLit { elements: elems } => {
                for e in elems {
                    sub(e);
                }
            }
    ...
    loop {
        ...
        for (empty_id, target, branch_args, param_values) in &redirects {
            ...
            for block in body.blocks.iter_mut() {
                ...
                // Update ControlHeader references
                if let Some(ref mut ctrl) = block.control {
                    match ctrl {
                        ControlHeader::Loop {
                            merge,
                            continue_block,
            ...
            // Kill the empty block
            body.blocks[empty_id.index()].terminator = None;
...
fn substitute_through(
    ...
    let param_map: HashMap<ValueId, ValueId> = from_params
        .iter()
        .zip(pred_args.iter())
        .map(|(&p, &a)| (p, a))
        .collect();
    ...
    to_args
        .iter()
        .map(|v| param_map.get(v).copied().unwrap_or(*v))
        .collect()
}
