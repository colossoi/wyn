...
fn try_parallelize_body(
    ...
    let (_storage_name, storage_local, array_ty) = match &map_info.source {
    ...
    // Create output local and add it to storage_buffer_wrappings
    // Output binding is after all input bindings
    let output_binding = inputs.len() as u32;
    let output_local = new_body.alloc_local(LocalDecl {
        name: "_output".to_string(),
        span: dummy_span,
        ty: array_ty.clone(),
        kind: LocalKind::Let, // Changed from Param - it will be bound to a StorageView
    });
    storage_buffer_wrappings.push((output_local, 0, output_binding, array_ty.clone()));
