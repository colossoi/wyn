...
#![deny(clippy::expect_used, clippy::let_underscore_must_use, clippy::unwrap_used)]
...
fn storage_buffer_name(binding: BindingRef, writable: bool, mixed: bool) -> String {
    let base = format!("_buf_{}_{}", binding.set, binding.binding);
    if !mixed {
        base
    } else if writable {
        format!("{base}_read_write")
    } else {
        format!("{base}_read")
    }
}
...
fn atomic_storage_bindings(program: &ssa::stage::WgslReady) -> LookupSet<BindingRef> {
    fn collect(body: &FuncBody, bindings: &mut LookupSet<BindingRef>) {
        for (_, atomic) in body.inner.insts.iter() {
            ...
            let view = body.inner.insts.iter().find_map(|(_, inst)| match &inst.data {
    ...
    for function in &program.functions {
        collect(&function.body, &mut bindings);
    }
    for entry in &program.entry_points {
        collect(&entry.body, &mut bindings);
    }
    bindings
}
struct LowerCtx<'a> {
...
impl<'a> LowerCtx<'a> {
    fn new(program: &'a ssa::stage::WgslReady, options: WgslOptions) -> Self {
        ...
        let atomic_bindings = atomic_storage_bindings(program);
        Self {
            program,
            function_variants,
    ...
    fn addressable_constant_expr(
        ...
        match &value.kind {
            ...
            AddressableConstantKind::Float(literal) => {
                let suffix = if matches!(&value.ty, PolyType::Constructed(TypeName::Float(16), _)) {
                    "h"
                } else {
                    "f"
                };
                Ok(
                    if literal.contains('.') || literal.contains('e') || literal.contains('E') {
                        format!("{literal}{suffix}")
                    } else {
                        format!("{literal}.0{suffix}")
                    },
                )
            }
    ...
    fn lower_program(&mut self) -> Result<String> {
        ...
        writeln!(output, "// Generated by the Wyn compiler — WGSL backend")?;
        if self.type_emitter.uses_f16() {
            writeln!(output, "enable f16;")?;
        }
        writeln!(output)?;
        ...
        for (name, elem) in &self.type_emitter.virtual_range_structs {
            writeln!(output, "struct {} {{", name)?;
            writeln!(output, "    f0: {},", elem)?;
            writeln!(output, "    f1: {},", elem)?;
            writeln!(output, "    f2: {},", elem)?;
            writeln!(output, "}}")?;
            writeln!(output)?;
        }
        ...
        for (name, elem, n) in &self.type_emitter.bounded_structs {
            writeln!(output, "struct {} {{", name)?;
            writeln!(output, "    f0: array<{}, {}>,", elem, n)?;
            writeln!(output, "    f1: i32,")?;
            writeln!(output, "}}")?;
            writeln!(output)?;
        }
        ...
        for (name, fields) in structs {
            ...
            for (field_name, attr, ty) in fields {
                writeln!(output, "    {}{}: {},", attr, field_name, ty)?;
            }
            writeln!(output, "}}")?;
            writeln!(output)?;
        }
        ...
        for b in blocks {
            writeln!(output, "struct {} {{", b.struct_name)?;
            for field in &b.fields {
                writeln!(output, "    {}: {},", field.wgsl_name, field.wgsl_type)?;
            }
            writeln!(output, "}}")?;
            writeln!(output)?;
            writeln!(
    ...
    fn storage_buffer_declarations(&mut self) -> Result<String> {
        ...
        for ((br, writable, elem_ty), entries) in synth_sorted {
            ...
            let elem_ty =
                if self.atomic_bindings.contains(&br) { format!("atomic<{}>", elem_ty) } else { elem_ty };
            let mixed = self
                .storage_access_variants
                .get(&br)
    ...
    fn lower_function(&mut self, func: &Function, emitted_name: &str, output: &mut String) -> Result<()> {
        ...
        if self.lowered.contains(emitted_name) {
            return Ok(());
        }
        self.lowered.insert(emitted_name.to_string());

        let body = &func.body;
        let name = self.mangle_tracked(emitted_name)?;
        ...
        writeln!(output, "fn {}({}) -> {} {{", name, params.join(", "), ret_ty)?;
        self.indent += 1;
        ...
        writeln!(output, "{}return {};", self.indent_str(), result)?;
    ...
    fn lower_entry_point(&mut self, entry: &EntryPoint, output: &mut String) -> Result<()> {
        ...
        writeln!(output, "{}", entry_attr)?;
        if is_compute_void {
            writeln!(output, "fn {}({}) {{", name, param_strs.join(", "))?;
        } else {
            writeln!(
                output,
                "fn {}({}) -> {} {{",
            ...
            match &multi_output_struct {
                ...
                None => {
                    for (i, out) in entry.outputs.iter().enumerate() {
                        ...
                        writeln!(output, "{}var {}: {};", self.indent_str(), name, ty_str)?;
        ...
        if !is_compute_void {
            if uses_output_ptrs {
                ...
                if multi_output_struct.is_some() {
                    writeln!(output, "{}return _out_struct;", self.indent_str())?;
                } else if output_locals.len() == 1 {
                    writeln!(output, "{}return _out0;", self.indent_str())?;
                } else {
                    return Err(err_wgsl!(
...
/// An emitted expression or a variable declared by an enclosing scope.
#[derive(Clone)]
enum ValueBinding {
...
impl ValueBinding {
    fn expr(&self) -> &str {
        match self {
            ValueBinding::Alias(s) | ValueBinding::Variable(s) => s,
        }
    }
}
...
struct BodyLowerCtx<'a, 'b> {
    ...
    uses: ssa::ValueUses,
    /// Emitted WGSL expression or previously declared variable per ValueId.
    value_map: LookupMap<ValueId, ValueBinding>,
    inline_values: LookupSet<ValueId>,
    next_copy: usize,
...
impl<'a, 'b> BodyLowerCtx<'a, 'b> {
    ...
    fn emit_ssa_binding(
        ...
        if matches!(self.value_map.get(&result), Some(ValueBinding::Variable(_))) {
            writeln!(output, "{}{} = {};", self.ctx.indent_str(), var, expr)?;
            return Ok(());
        } else if self.needs_mutable_binding.contains(&result) {
    ...
    fn emit_mutable_binding(
        ...
        self.needs_mutable_binding.insert(result);
        self.emit_ssa_binding(output, result, ty, expr)?;
        Ok(wgsl_var(result))
    }

    /// An alias may omit a binding only within the scope analyzed for it.
    /// Values exported from a lexical block need their planned storage.
    fn emit_alias_or_binding(&mut self, output: &mut String, result: ValueId, expr: String) -> Result<()> {
        if self.value_map.get(&result).is_some_and(|binding| binding.expr() == expr) {
            return Ok(());
        }
        if self.inline_values.contains(&result) {
    ...
    fn should_inline(&self, value: ValueId, inst: &WynInstNode, expr: &str) -> bool {
        ...
        self.inline_values.contains(&value)
    ...
    fn lower(&mut self, output: &mut String) -> Result<String> {
        ...
        let mut tree = structured::structurize(self.body);
        self.inline_values = super::bindings::place_bindings(self.body, &mut tree);
        self.emit_scope(&tree.root, output)
    }

    fn emit_scope(&mut self, scope: &structured::Scope, output: &mut String) -> Result<String> {
        for &value in &scope.declarations {
            let var = wgsl_var(value);
            let ty = self.ctx.type_emitter.type_to_wgsl(self.body.get_value_type(value))?;
            writeln!(output, "{}var {}: {};", self.ctx.indent_str(), var, ty)?;
            self.addressable.insert(var.clone());
            self.writable.insert(var.clone());
            self.value_map.insert(value, ValueBinding::Variable(var));
        }
        self.emit_nodes(&scope.nodes, output)
    }
        ...
        for node in nodes {
            match node {
                Node::Inst(inst_id) => {
                    ...
                    match &inst.data {
                        ...
                        InstKind::Load { place } => {
                            ...
                            let target = if self.is_atomic_place(*place) {
                        ...
                        InstKind::Atomic { place, op, values } => {
                            ...
                            let expression = match (op, values.as_slice()) {
                                ...
                                (AtomicOp::SignedMax | AtomicOp::UnsignedMax, [value]) => {
                                    format!("atomicMax(&{target}, {value})")
                                }
                                (AtomicOp::And, [value]) => format!("atomicAnd(&{target}, {value})"),
                                (AtomicOp::Or, [value]) => format!("atomicOr(&{target}, {value})"),
                                (AtomicOp::Xor, [value]) => format!("atomicXor(&{target}, {value})"),
                            ...
                            self.emit_ssa_binding(output, result_id, &ty, &expression)?;
                            continue;
                        }

                        InstKind::ControlBarrier => {
                            writeln!(output, "{}workgroupBarrier();", self.ctx.indent_str())?;
                            continue;
                        }
                        ...
                        InstKind::Op {
                            ...
                            let ty =
                                self.ctx.type_emitter.type_to_wgsl(self.body.get_value_type(result_id))?;
                            self.emit_mutable_binding(output, result_id, &ty, &format!("{ty}()"))?;
                            continue;
                        }
                            ...
                            || *id == catalog().known().array_with =>
                        {
                            ...
                            if is_inplace {
                                ...
                                if let Some(target) = view_target {
                                    writeln!(output, "{}{} = {};", self.ctx.indent_str(), target, val)?;
                                    self.emit_alias_or_binding(output, result_id, arr_src)?;
                                } else {
                                    ...
                                    let target = if let Some(alias) = writable_alias {
                                        ...
                                        self.emit_mutable_binding(output, result_id, &ty, &arr_src)?
                                    };
                                    writeln!(
                                        output,
                                        "{}{}[{}] = {};",
                                    ...
                                    self.emit_alias_or_binding(output, result_id, target)?;
                                }
                            } else {
                                let ty = self
                                    .ctx
                                    .type_emitter
                                    .type_to_wgsl(self.body.get_value_type(result_id))?;
                                let var = self.emit_mutable_binding(output, result_id, &ty, &arr_src)?;
                        ...
                        InstKind::Op {
                            ...
                            if let Some(alias) = addressable_alias.filter(|_| {
                                !matches!(self.value_map.get(&result_id), Some(ValueBinding::Variable(_)))
                            }) {
                                self.value_map.insert(result_id, ValueBinding::Alias(alias));
                            } else {
                                let val = self.get_value(operands[0])?;
                                self.emit_mutable_binding(output, result_id, &ty, &val)?;
                            }
                            continue;
                        }
                    ...
                    if let Some(result) = inst.result {
                        ...
                        if matches!(
                            ...
                            self.emit_alias_or_binding(output, result, expr)?;
                            continue;
                        }
                        ...
                        if matches!(
                            ...
                            writeln!(output, "{}{};", self.ctx.indent_str(), expr)?;
                            continue;
                        }
                        ...
                        if (is_scalar_literal(inst) && self.inline_values.contains(&result))
                            || self.should_inline(result, inst, &expr)
                        {
                            self.value_map.insert(result, ValueBinding::Alias(expr));
                            continue;
                        }
                ...
                Node::ParallelCopy(copies) => {
                    ...
                    // Snapshot all sources before updating any destination.
                    // This also protects expressions that reference mutable
                    // loop state through an inlined alias.
                    for &(target, value) in copies {
                        let mut expr = self.get_value(value)?;
                        if copies.len() > 1 {
                            let temporary = format!("_copy{}", self.next_copy);
                            self.next_copy += 1;
                            let ty =
                                self.ctx.type_emitter.type_to_wgsl(self.body.get_value_type(target))?;
                            writeln!(
                                output,
                                "{}let {}: {} = {};",
                                self.ctx.indent_str(),
                ...
                Node::If {
                    ...
                    writeln!(output, "{}if {} {{", self.ctx.indent_str(), cond_val)?;
                    self.ctx.indent += 1;
                    self.emit_scope(then_body, output)?;
                    self.ctx.indent -= 1;
                    writeln!(output, "{}}} else {{", self.ctx.indent_str())?;
                    self.ctx.indent += 1;
                    self.emit_scope(else_body, output)?;
                    self.ctx.indent -= 1;
                    writeln!(output, "{}}}", self.ctx.indent_str())?;
                }

                Node::Loop { body } => {
                    writeln!(output, "{}loop {{", self.ctx.indent_str())?;
                    self.ctx.indent += 1;
                    self.emit_scope(body, output)?;
                    self.ctx.indent -= 1;
                    writeln!(output, "{}}}", self.ctx.indent_str())?;
                }

                Node::BreakIf { cond, when_true } => {
                    let condition = self.get_value(*cond)?;
                    let negation = if *when_true { "" } else { "!" };
                    writeln!(
                        output,
                        "{}if {}({}) {{ break; }}",
                        self.ctx.indent_str(),
            ...
            InstKind::Op { tag, operands } => match tag {
                ...
                op::OpTag::Float(s) => {
                    let suffix = if matches!(
                        result_ty.as_ref(),
                        Some(PolyType::Constructed(TypeName::Float(16), _))
                    ) {
                        "h"
                    } else {
                        "f"
                ...
                op::OpTag::Intrinsic { id, overload_idx } => {
                    ...
                    if *id == known.length && args.len() == 1 {
                        if let Some(id) = args[0].as_ssa() {
                            ...
                            // field of the range struct, element-typed. Length
                            // consumers may request either index type, so cast
                            // whenever the range element and result signedness
                            // differ.
                            if let Some(PolyType::Constructed(TypeName::ArrayVariantVirtual, _)) =
                                ty.array_variant()
                            {
                                let expr = format!("{}.f2", arg_strs[0]);
                                let range_len_is_i32 = matches!(
                                    ty.elem_type(),
                                    Some(PolyType::Constructed(TypeName::Int(32), _))
                                );
                                return Ok(match (range_len_is_i32, wants_i32) {
                                    (true, false) => format!("u32({expr})"),
                                    (false, true) => format!("i32({expr})"),
                                    _ => expr,
                                });
                            }
