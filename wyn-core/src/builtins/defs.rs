...
static STATIC_BUILTINS: &[BuiltinDefRaw] = &[
    ...
    // Storage-image ops. Compute-side linear writes use
    // `img with [xy] = rgba` (lowered through `_w_intrinsic_image_with`);
    // point reads lower to `OpImageRead`. Filtering (bilinear sampling) of
    // a storage image goes through `texture_sample` on a `Texture2D` view of
    // the same underlying GPU resource — the host wires both bindings to one
    // wgpu texture.
    polymorphic_intrinsic!(
        "image_load",
    ...
    BuiltinDefRaw {
        surface_name: INTRINSIC_IMAGE_WITH,
        intrinsic_source_names: &[],
        impl_source_names: &[INTRINSIC_IMAGE_WITH],
        kind: BuiltinKind::InternalIntrinsic,
        purity: Purity::Effectful,
        overloads: &[BuiltinOverload {
            scheme: Some(image_with_scheme),
            lowering: BuiltinLowering::ByBuiltinId,
        }],
    },
    polymorphic_intrinsic!(
