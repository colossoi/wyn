...
use super::{
    ...
    Pattern, PatternKind, RangeExpr, RecordPatternField, SliceExpr, TreeFamily,
};
    ...
    To: TreeFamily,
{
    ...
    let kind = expression_kind(kind, &h, header, identifier, type_hole)?;
    ...
    To: TreeFamily,
{
    Ok(match kind {
        ...
        ExprKind::ArrayLiteral(values) => ExprKind::ArrayLiteral(
            values
                .into_iter()
                .map(|value| expression(value, header, identifier, type_hole))
                .collect::<Result<_, _>>()?,
        ),
        ...
        ExprKind::ArrayIndex(array, index) => ExprKind::ArrayIndex(
            Box::new(expression(*array, header, identifier, type_hole)?),
            Box::new(expression(*index, header, identifier, type_hole)?),
        ),
        ExprKind::ArrayWith { array, index, value } => ExprKind::ArrayWith {
            array: Box::new(expression(*array, header, identifier, type_hole)?),
            index: Box::new(expression(*index, header, identifier, type_hole)?),
            value: Box::new(expression(*value, header, identifier, type_hole)?),
        },
        ExprKind::VecWith {
            target,
            components,
            ...
            target: Box::new(expression(*target, header, identifier, type_hole)?),
            components,
            op,
            value: Box::new(expression(*value, header, identifier, type_hole)?),
        },
        ExprKind::RecordWith { record, path, value } => ExprKind::RecordWith {
            record: Box::new(expression(*record, header, identifier, type_hole)?),
            path,
            value: Box::new(expression(*value, header, identifier, type_hole)?),
        },
        ExprKind::BinaryOp(op, left, right) => ExprKind::BinaryOp(
            op,
            Box::new(expression(*left, header, identifier, type_hole)?),
            Box::new(expression(*right, header, identifier, type_hole)?),
        ),
        ExprKind::UnaryOp(op, value) => {
            ExprKind::UnaryOp(op, Box::new(expression(*value, header, identifier, type_hole)?))
        }
        ...
        ExprKind::RecordLiteral(fields) => ExprKind::RecordLiteral(
            fields
                .into_iter()
                .map(|(name, value)| Ok((name, expression(value, header, identifier, type_hole)?)))
                .collect::<Result<_, E>>()?,
        ),
        ExprKind::Lambda(lambda) => ExprKind::Lambda(LambdaExpr {
            params: lambda
                .params
                .into_iter()
                .map(|value| pattern(value, header))
                .collect::<Result<_, _>>()?,
            body: Box::new(expression(*lambda.body, header, identifier, type_hole)?),
        }),
        ExprKind::Application(function, arguments) => ExprKind::Application(
            Box::new(expression(*function, header, identifier, type_hole)?),
            arguments
                .into_iter()
                .map(|value| expression(value, header, identifier, type_hole))
                .collect::<Result<_, _>>()?,
        ...
        ExprKind::LetIn(let_in) => ExprKind::LetIn(LetInExpr {
            pattern: pattern(let_in.pattern, header)?,
            ty: let_in.ty,
            value: Box::new(expression(*let_in.value, header, identifier, type_hole)?),
            body: Box::new(expression(*let_in.body, header, identifier, type_hole)?),
        }),
        ExprKind::FieldAccess(value, field) => ExprKind::FieldAccess(
            Box::new(expression(*value, header, identifier, type_hole)?),
            field,
        ),
        ...
        ExprKind::Loop(loop_expression) => ExprKind::Loop(LoopExpr {
            pattern: pattern(loop_expression.pattern, header)?,
            init: loop_expression
                .init
                .map(|value| expression(*value, header, identifier, type_hole).map(Box::new))
                .transpose()?,
            form: loop_form(loop_expression.form, header, identifier, type_hole)?,
            body: Box::new(expression(*loop_expression.body, header, identifier, type_hole)?),
        }),
        ExprKind::Match(match_expression) => ExprKind::Match(MatchExpr {
            ...
            cases: match_expression
                ...
                .map(|case| {
                    Ok(MatchCase {
                        pattern: pattern(case.pattern, header)?,
                        body: Box::new(expression(*case.body, header, identifier, type_hole)?),
                    })
                })
                .collect::<Result<_, E>>()?,
        }),
        ...
        ExprKind::Range(range) => ExprKind::Range(RangeExpr {
            start: Box::new(expression(*range.start, header, identifier, type_hole)?),
            step: range
                .step
                .map(|value| expression(*value, header, identifier, type_hole).map(Box::new))
                .transpose()?,
            end: Box::new(expression(*range.end, header, identifier, type_hole)?),
            kind: range.kind,
        }),
        ExprKind::Slice(slice) => ExprKind::Slice(SliceExpr {
            array: Box::new(expression(*slice.array, header, identifier, type_hole)?),
            start: slice
                .start
                .map(|value| expression(*value, header, identifier, type_hole).map(Box::new))
                .transpose()?,
        ...
        ExprKind::TypeAscription(value, ty) => {
            ExprKind::TypeAscription(Box::new(expression(*value, header, identifier, type_hole)?), ty)
        }
        ExprKind::TypeCoercion(value, ty) => {
            ExprKind::TypeCoercion(Box::new(expression(*value, header, identifier, type_hole)?), ty)
        }
    })
}
    ...
    To: TreeFamily,
{
    Ok(match form {
        LoopForm::For(name, bound) => {
            LoopForm::For(name, Box::new(expression(*bound, header, identifier, type_hole)?))
        }
        LoopForm::ForIn(item, iterable) => LoopForm::ForIn(
            pattern(item, header)?,
            Box::new(expression(*iterable, header, identifier, type_hole)?),
        ),
        LoopForm::While(condition) => {
            LoopForm::While(Box::new(expression(*condition, header, identifier, type_hole)?))
        }
    })
}

pub fn pattern<FromHeader, ToHeader, A, E>(
    pattern: Pattern<FromHeader, A>,
    header: &mut impl FnMut(FromHeader) -> Result<ToHeader, E>,
) -> Result<Pattern<ToHeader, A>, E> {
    ...
    let kind = match kind {
        ...
        PatternKind::Tuple(patterns) => PatternKind::Tuple(
            patterns.into_iter().map(|value| self::pattern(value, header)).collect::<Result<_, _>>()?,
        ),
            ...
            fields
                ...
                .map(|field| {
                    Ok(RecordPatternField {
                        field: field.field,
                        pattern: field.pattern.map(|value| self::pattern(value, header)).transpose()?,
                    })
                })
                .collect::<Result<_, E>>()?,
        ),
        ...
        PatternKind::Typed(pattern, ty) => {
            PatternKind::Typed(Box::new(self::pattern(*pattern, header)?), ty)
        }
        PatternKind::Attributed(attributes, pattern) => {
            PatternKind::Attributed(attributes, Box::new(self::pattern(*pattern, header)?))
        }
    };
    Ok(Node { h: header(h)?, kind })
}
