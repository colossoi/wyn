...
fn reorder_expr(
    ...
    let new_expr = match old_body.get_expr(old_id).clone() {
        ...
        Expr::StorageView { set, binding, offset, len } => {
            ...
            Expr::StorageView { set, binding, offset: new_offset, len: new_len }
        }
...
fn exprs_equal(body: &Body, a: ExprId, b: ExprId) -> bool {
    ...
    match (body.get_expr(a), body.get_expr(b)) {
        ...
        (
            Expr::StorageView { set: sa, binding: ba, offset: oa, len: la },
            Expr::StorageView { set: sb, binding: bb, offset: ob, len: lb },
        ) => {
            sa == sb && ba == bb && exprs_equal(body, *oa, *ob) && exprs_equal(body, *la, *lb)
        }
        (
            Expr::SliceStorageView { view: va, start: sa, len: la },
            Expr::SliceStorageView { view: vb, start: sb, len: lb },
        ) => {
