...
impl<'a> TypeChecker<'a> {
    ...
    fn check_decl_as_in_module(
        ...
        if decl.params.is_empty() {
            ...
            // uniqueness can be discarded at the return boundary, so use
    ...
    /// stripping a leading `*` from `actual` only when `expected` is *not*
    /// unique. Use this at coercion sites where the `expected` side is a
    /// contract and the `actual` side is a value being coerced to fit it —
    ...
    ///   - expected `T`,  actual `T`   → no strip, unify directly
    ///   - expected `*T`, actual `T`   → no strip; unification fails (you
    ///                                    can't manufacture uniqueness)
    ///
    /// **Do not** use this where `actual` and `expected` are symmetric
    /// (two branch results, two array elements being unified to a common
    /// type). For those, use plain `unify_or_err` so neither side
    /// silently loses uniqueness.
