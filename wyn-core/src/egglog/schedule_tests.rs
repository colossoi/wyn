...
fn kernel_count(data: &AssociatedData) -> usize {
    data.blocks
        .values()
...
fn fused_map_chain_becomes_one_guarded_kernel_with_explicit_captures() {
    ...
    assert_eq!(kernel_count(&result.data), 1);
    assert_eq!(result.data.dispatches.len(), 1);
    for n in [0, 1, 63, 64, 65, 137] {
        let input: Vec<_> = (0..n).collect();
        let output = run(
            &result.data,
            vec![Value::array(input.iter().copied()), Value::Int(7)],
        );
        assert_eq!(
            output[0].ints(),
...
fn reduction_handles_empty_tail_chunks_and_tuple_accumulators() {
    ...
    assert_eq!(kernel_count(&result.data), 2);
    for n in [0, 1, 63, 64, 65, 137, 4097] {
        let output = run(&result.data, vec![Value::array(0..n)]);
        assert_eq!(
            output[0],
...
fn scan_and_filter_have_global_dispatch_boundaries_and_correct_results() {
    ...
    assert_eq!(kernel_count(&scan.data), 3);
    assert_eq!(kernel_count(&filter.data), 4);
    for result in [&scan, &filter] {
        let ds: Vec<_> = result.data.dispatches.ids().collect();
        for pair in ds.windows(2) {
            assert!(result.data.dispatches[pair[1]].dependencies.contains(&pair[0]));
        }
    }
    for n in [0, 1, 63, 64, 65, 137] {
        let output = run(&scan.data, vec![Value::array(0..n)]);
        assert_eq!(
            output[0].ints(),
            (0..n).map(|i| i * (i + 1) / 2).collect::<Vec<_>>()
        );
        let output = run(&filter.data, vec![Value::array(0..n)]);
        assert_eq!(
            output[0].ints(),
...
fn conditional_dispatches_execute_only_the_selected_arm() {
    ...
    assert_eq!(kernel_count(&result.data), 4);
    assert_eq!(
        result.data.dispatches.values().filter(|d| d.dependencies.is_empty()).count(),
        2
    );
    for flag in [false, true] {
        let output = run(&result.data, vec![Value::array(0..5), Value::Bool(flag)]);
        assert_eq!(
            output[0].ints(),
...
fn noncommutative_associative_reduction_preserves_chunk_order() {
    ...
    for n in [0, 65, 137] {
        ...
        let result = run(&result.data, vec![Value::array(0..n)]);
        assert_eq!(
            result[0],
...
fn ordered_writes_and_ranked_bucket_overflow_lower_to_ordinary_memory_ops() {
    ...
    let output = run(&scatter.data, vec![Value::array([10, 20, 30])]);
...
fn nested_array_work_uses_device_loops_without_device_launches() {
    ...
    assert_eq!(kernel_count(&result.data), 1);
    let output = run(&result.data, vec![Value::array(0..7)]);
    assert_eq!(output[0].ints(), (0..7).map(|x| 10 + x * 5).collect::<Vec<_>>());
    for block in result.data.blocks.values() {
        let kind = &result.data.blocks[block.function].interface.as_ref().unwrap().kind;
        if matches!(kind, FunctionKind::Kernel(_) | FunctionKind::Device) {
            assert!(!result.data.bodies[block.body]
                .instructions
                .iter()
    ...
    for body in result.data.bodies.values() {
        for instruction in &body.instructions {
            if let Instruction::Evaluate(op) = instruction {
                assert!(matches!(
                    result.data.operations[*op].kind,
...
fn verifier_rejects_incorrect_block_arguments_and_cyclic_dispatches() {
    ...
    let first = result.data.dispatches.ids().next().unwrap();
    result.data.dispatches[first].dependencies.insert(first);
    assert!(validation::validate(&result.data).unwrap_err().to_string().contains("cyclic"));
    result.data.dispatches[first].dependencies.remove(&first);
    let edge = result
        .data
        .blocks
        .values()
        ...
        .unwrap();
    result.data.bodies[edge.arguments].results.clear();
    assert!(validation::validate(&result.data).unwrap_err().to_string().contains("arity"));
}
...
fn grid_stride_kernels_cover_more_work_than_the_launched_invocations() {
    for source in [
        ...
        for id in result.data.grids.ids().collect::<Vec<_>>() {
            result.data.grids[id].groups[0] = super::Value::Int(1);
        }
        let output = run(&result.data, vec![Value::array(0..137)]);
...
fn nested_source_loops_keep_distinct_accumulators_and_header_bindings() {
    ...
    for n in [0, 1, 3] {
        let output = run(&result.data, vec![Value::array(0..4), Value::Int(n)]);
...
fn final_roots_exclude_dead_functions_and_pure_array_work() {
    ...
    assert_eq!(kernel_count(&result.data), 0);
    assert_eq!(run(&result.data, vec![Value::array(0..5)]), [Value::Int(7)]);
    let functions: Vec<_> = result.data.blocks.values().filter_map(|b| b.interface.as_ref()).collect();
