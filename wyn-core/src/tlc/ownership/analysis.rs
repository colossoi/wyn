...
/// Mutable workspace shared by effect construction, liveness, validation, and
/// late patch selection within one ownership pass invocation.
///
...
pub(super) struct AnalysisState {
    ...
    pub(super) var_to_owner: LookupMap<SymbolId, OwnerId>,
    ...
    pub(super) var_aliases: LookupMap<SymbolId, LookupSet<OwnerId>>,
    ...
    /// overwritten.
    pub(super) owner_to_var: LookupMap<OwnerId, SymbolId>,
    /// Per-owner provenance.
    pub(super) origins: LookupMap<OwnerId, Origin>,
    /// Per-term span — for error reporting.
    pub(super) term_spans: LookupMap<TermId, Span>,
    /// Per-term: owners read here.
    pub(super) uses: LookupMap<TermId, LookupSet<OwnerId>>,
    /// Per-term: owners consumed/moved here (e.g. function arg with
    /// `*T` parameter type).
    pub(super) kills: LookupMap<TermId, LookupSet<OwnerId>>,
    /// Per-term: owners newly introduced here.
    pub(super) defs: LookupMap<TermId, LookupSet<OwnerId>>,
    /// Per-term: live-out set. Empty until liveness runs.
    pub(super) live_out: LookupMap<TermId, LookupSet<OwnerId>>,
...
impl AnalysisState {
    ...
    pub(super) fn owner_of(&self, sym: SymbolId) -> Option<OwnerId> {
        self.var_to_owner.get(&sym).copied()
    }

    pub(super) fn aliases_of(&self, sym: SymbolId) -> LookupSet<OwnerId> {
    ...
    pub(super) fn origin(&self, owner: OwnerId) -> Option<Origin> {
...
struct Builder<'p, C: Payload, S: Payload> {
    model: AnalysisState,
    next_owner: u32,
    symbols: &'p SymbolTable,
    /// Def symbol → `(parameter diets, return diet)`.
    def_diets: LookupMap<SymbolId, (Vec<Diet>, Diet)>,
    /// Memoized `alias_value` results. Term IDs are program-global; the cache
    /// is nevertheless cleared per definition so values never outlive the
    /// binding environment in which they were interpreted.
...
impl<'p, C: Payload, S: Payload> Builder<'p, C, S> {
    fn new(symbols: &'p SymbolTable, def_diets: LookupMap<SymbolId, (Vec<Diet>, Diet)>) -> Self {
        Self {
            model: AnalysisState::new(),
            next_owner: 0,
            symbols,
            def_diets,
    ...
    fn callee_diet(&self, func: &Term<C, S>) -> Option<(&[Diet], &Diet)> {
        if let TermKind::Var(VarRef::Symbol(sym)) = &func.kind {
            if let Some((param_diets, return_diet)) = self.def_diets.get(sym) {
                return Some((param_diets, return_diet));
            }
        }
        None
    }
    ...
    fn callee_return_diet(&self, func: &Term<C, S>) -> Diet {
        ...
        if var_term_builtin_id(func, self.symbols) == Some(known.image_with) {
    ...
    fn producer_shape(&self, rhs: &Term<C, S>) -> (Type<TypeName>, Diet) {
        match &rhs.kind {
    ...
    fn visit_def<E: Clone + std::fmt::Debug>(
        &mut self,
        body: &Term<C, S>,
        meta: &DefMeta<E>,
        param_diets: &[Diet],
        ...
        // Keep cached alias interpretation scoped to this definition's
        // binding environment. Term IDs themselves are program-global.
        ...
        if let TermKind::Lambda(lam) = &body.kind {
    ...
    fn check_alias_free_return(&mut self, ret_ty: &Type<TypeName>, ret_diet: &Diet, body: &Term<C, S>) {
    ...
    fn bind_params(&mut self, lam: &Lambda<C, S>, param_diets: &[Diet], is_entry: bool) {
    ...
    /// visited.
    fn visit_term(&mut self, term: &Term<C, S>) {
                        ...
                        if !aliases.is_empty() {
                            ...
                            if !rejected.is_empty() {
                                for owner in rejected {
                                    let var_name = owner_display_name(&self.model, self.symbols, owner);
    ...
    fn visit_lambda(&mut self, lam: &Lambda<C, S>) {
    ...
    fn visit_soac_body(&mut self, sb: &SoacBody<C, S>) {
        S::for_each(&sb.data, &mut |capture| {
            let (capture_sym, capture_ty, capture_term) = capture;
    ...
    fn visit_soac(&mut self, op: &SoacOp<C, S>, soac_id: TermId) {
        match op {
    ...
    fn bind_reducer_params(&mut self, op: &Lambda<C, S>, input: &ArrayExpr<C, S>, soac_id: TermId) {
    ...
    fn visit_array_expr(&mut self, ae: &ArrayExpr<C, S>) {
        match ae {
    ...
    fn alias_targets(&mut self, term: &Term<C, S>) -> LookupSet<OwnerId> {
        self.alias_value(term).flattened()
    }

    fn alias_value(&mut self, term: &Term<C, S>) -> AliasValue {
    ...
    fn compute_alias_value(&mut self, term: &Term<C, S>) -> AliasValue {
        match &term.kind {
            ...
            TermKind::App { func, args } => {
                let known = catalog().known();
                if var_term_builtin_id(func, self.symbols) == Some(known.array_with_in_place) {
                    return args
                        .first()
            ...
            TermKind::Closure(data) => {
                let mut aliases = LookupSet::new();
                C::for_each(data, &mut |capture| aliases.extend(self.alias_targets(capture)));
                AliasValue::leaf(aliases)
            }
            TermKind::Lambda(_)
            | TermKind::IntLit(_)
            | TermKind::FloatLit(_)
    ...
    fn allocates_fresh_outer(&self, term: &Term<C, S>, _ty: &Type<TypeName>) -> bool {
        match &term.kind {
            TermKind::ArrayExpr(ArrayExpr::Literal(_) | ArrayExpr::Range { .. }) => true,
            TermKind::Soac(SoacOp::Map { .. } | SoacOp::Scan { .. } | SoacOp::Filter { .. }) => true,
            TermKind::App { func, .. } => {
                var_term_builtin_id(func, self.symbols) == Some(catalog().known().array_with)
            }
            _ => false,
        }
    }

    fn aliases_of_terms<'a>(
        &mut self,
        terms: impl IntoIterator<Item = &'a Term<C, S>>,
    ) -> LookupSet<OwnerId>
    where
        C: 'a,
        S: 'a,
    ...
    fn array_expr_aliases(&self, ae: &ArrayExpr<C, S>) -> LookupSet<OwnerId> {
        match ae {
    ...
    fn aliases_of_array_exprs(&self, aes: &[ArrayExpr<C, S>]) -> LookupSet<OwnerId> {
        let mut aliases = LookupSet::new();
        for ae in aes {
    ...
    fn is_definitely_alias_free(&mut self, term: &Term<C, S>) -> bool {
    ...
    fn origin_for_unaliased(&mut self, rhs: &Term<C, S>) -> Origin {
...
pub(super) fn owner_display_name(model: &AnalysisState, symbols: &SymbolTable, owner: OwnerId) -> String {
    model
        .owner_to_var
        .get(&owner)
        .and_then(|s| symbols.get(*s).cloned())
...
pub(super) fn alias_target_of<C: Payload, S: Payload>(
    term: &Term<C, S>,
    model: &AnalysisState,
    symbols: &SymbolTable,
) -> Option<OwnerId> {
    match &term.kind {
        ...
        // aliases the base.
        TermKind::Index { array, .. } => alias_target_of(array, model, symbols),
        TermKind::TupleProj { tuple, .. } => alias_target_of(tuple, model, symbols),
        // In-place `array_with` returns the buffer it consumed.
        TermKind::App { func, args } => {
            if var_term_builtin_id(func, symbols) == Some(catalog().known().array_with_in_place) {
                return alias_target_of(args.first()?, model, symbols);
            }
            None
        }
        _ => None,
