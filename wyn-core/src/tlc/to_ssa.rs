...
fn convert_function(
    ...
    if let TermKind::Extern(linkage_name) = &def.body.kind {
        ...
        let params: Vec<(Type<TypeName>, String)> = param_types
            .into_iter()
            .enumerate()
    ...
    let param_list: Vec<(Type<TypeName>, String)> = params
        .iter()
        .map(|(name, ty, _)| (ty.clone(), name.clone()))
        .collect();
...
impl<'a> Converter<'a> {
    ...
    fn convert_app(
        ...
        match &base_term.kind {
            ...
            TermKind::Var(name) => {
                self.convert_var_application(name, &args, ty, span, node_id)
            }
    ...
    fn convert_loop(
        ...
        match kind {
            TlcLoopKind::While { cond } => {
                self.convert_while_loop(loop_var, loop_var_ty, init, init_bindings, cond, body, span, node_id)
            }
    ...
    fn convert_reduce(
        ...
        let elem_ty = match &arr_term.ty {
            Type::Constructed(TypeName::Array, type_args) if !type_args.is_empty() => {
                type_args[0].clone()
            }
            _ => acc_ty.clone(),
        };
