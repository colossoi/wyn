//! Diagnostic utilities for AST and TLC formatting and display.
//!
...
fn format_constructed_type(name: &TypeName, args: &[PolyType<TypeName>]) -> String {
    match name {
        ...
        TypeName::Resource(resource) => format!("resource({})", resource.index()),
...
impl<C: tlc::Payload, S: tlc::Payload> tlc::Term<C, S> {
    fn fmt_prec(&self, f: &mut Formatter<'_>, prec: usize) -> fmt::Result {
        match &self.kind {
            ...
            tlc::TermKind::Soac(_) | tlc::TermKind::ArrayExpr(_) => {
                write!(f, "<soac>")
            }
