...
pub(super) fn seg_extent_fusable<R: GraphResource>(a: &SegExtent<R>, b: &SegExtent<R>) -> bool {
    match (a, b) {
        ...
        // Dependency lists do not prove that two unknown calculations are
        // equal. Shared value identity does.
        (SegExtent::HostProvided { node: x, .. }, SegExtent::HostProvided { node: y, .. }) => x == y,
