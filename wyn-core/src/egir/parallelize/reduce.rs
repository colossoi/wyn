...
pub(super) struct ReduceAnalysis {
...
pub(super) fn bind_reduce_candidate(
    candidate: ReduceCandidate,
    resources: &planning::ResourceIndex<'_>,
) -> error::Result<ReduceAnalysis> {
    let partials = resources
        ...
        .collect();
    Ok(ReduceAnalysis { candidate, partials })
}
...
fn synthesize_phase2_reduce_cloning_ne_named(
    ...
    accumulator_value: NodeId,
    output_stores: &[(NodeId, NodeId)],
    output_decls: &[(ResourceId, Type<TypeName>, crate::egir::program::LogicalSize)],
    phase2_width: u32,
    effect_ids: &mut crate::IdSource<EffectToken>,
